...
function showCategories($start,$limit)
{
	...
	quiz_collapsableBar('newsub', 'topnewsubicon');
		...
		$goImage = "<img src= \"".ICMS_URL."/modules/xquiz/images/cat.gif \" title="._QUIZ_EDIT." alt='' >";
		...
		{						
			...
			<tr class='".$class."'>
				...
				.$goImage.
				"
					<a href=\"".ICMS_URL."/modules/xquiz/index.php?cid=".$key['cid']."\">".$key['title']."</a>
				</td>
				<td>
				"
				.$key['weight'].
				...
				<td>
				<a href=\"".ICMS_URL."/modules/xquiz/admin/index.php?op=Cate&act=del&Id=".$key['cid']."\">
				".
				$delImage
				."
				</a>
				<a href=\"".ICMS_URL."/modules/xquiz/admin/index.php?op=Cate&act=edit&Id=".$key['cid']."\">
				".
				$editImage
				."
				</td>
		...
		$nav  = new icms_view_PageNav($nume ,$limit ,$start ,'start','op=Cate' );
...
function CategoryForm($op = "add" ,$eId = 0)
    ...
    $topics_array = XoopsLists :: getImgListAsArray( ICMS_ROOT_PATH . '/modules/xquiz/images/topics/' );
    foreach( $topics_array as $image ) {
        $imageselect->addOption("$image", $image);
    }
	$imageselect->setExtra( "onchange='showImgSelected(\"image3\", \"topic_imgurl\", \"" . $uploadirectory . "\", \"\", \"" . ICMS_URL . "\")'" );
    $imgtray->addElement($imageselect,false);
    $imgtray -> addElement( new XoopsFormLabel( '', "<br /><img src='" . ICMS_URL . "/" . $uploadirectory . "/" . $topicimage . "' name='image3' id='image3' alt='' />" ) );

	$uploadfolder=sprintf(_AM_UPLOAD_WARNING,ICMS_URL . '/modules/' . $xoopsModule -> dirname().'/images/topics');
